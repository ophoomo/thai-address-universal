...
/**
 * Caches for storing information related to provinces, districts, sub-districts, and zip codes.
 *
 ...
 * - `zipCodeCache`: A Map that stores arrays of zip codes indexed by a string key (e.g., sub-district name).
 */
...
const loadThaiDatabase = () => {
    import('../migrate/output/th_db.json')
        .then((data) => {
            thaiWords = preprocess_word(data, true);
            thaiDB = preprocess(thaiAddressDB as IProvince[], thaiWords);
            isThaiDBLoaded = true;
        })
        .catch((err) => console.error('Error load eng word', err));
};

const loadEngDatabase = () => {
    import('../migrate/output/en_db.json')
        .then((data) => {
 ...
 *
 * @returns {IExpanded[]} The corresponding expanded database (either en_db or th_db).
 */
const db = (): IExpanded[] => {
    setupDatabase();
 ...
 * @returns {IExpanded[]} - A filtered array of items from the database that match the search criteria.
 */
...
/**
 * Retrieves all unique zip codes for a given sub-district.
 * If the zip code data for the specified sub-district is not already cached, it will extract the zip codes from the database and store them in the cache.
 *
 * @param sub_district - The name of the sub-district for which to retrieve the zip codes.
 *
 * @returns A list of unique zip codes (strings) for the specified sub-district.
 */
    ...
    cacheResult(zipCodeCache, sub_district, () => {
        ...
        db().forEach((item) => {
            if (item.sub_district === sub_district)
                zipCodeSet.add(item.postal_code);
        });
        return zipCodeSet;
    });
...
/**
 * Searches for addresses by zip code based on the provided search string or number.
 * This function internally calls `searchAddressByField` with the 'zipcode' field and the given search string or number.
 * Optionally, it limits the number of results to the specified maximum.
 *
 * @param searchStr - The search string or number to match in the zip code field.
 * @param maxResult - An optional parameter that specifies the maximum number of results to return.
 *
 * @returns An array of addresses (IExpanded[]) that match the search criteria in the zip code field.
 */
export const searchAddressByZipCode = (
    searchStr: string | number,
    maxResult?: number,
): IExpanded[] => searchAddressByField('postal_code', searchStr, maxResult);

/**
 * Splits a full address string into its components and returns an expanded address object.
 * This function extracts the zip code from the full address, uses it to search for a matching address in the database,
 * and then returns a structured object containing the address components such as province, district, sub-district, and zip code.
 *
